# Handover log, through 2026-08-15
Status: stale

> ARCHIVED 2026-08-17. This grew to 918 lines by being APPENDED to, which `conducks-docs` §6.11
> forbids — a handover is overwritten each session, two sections, fifteen lines — and §6.12 forbids a
> progress file outright. It also declared `Status: current` while describing `conducks fallback` and
> `audit --fallback`, commands removed on 2026-08-17, and quoting 1,857 tests against a suite that
> now runs 2,319. `docs-status` called it current the whole time, because it reads the STATUS LINE
> and not the content.
>
> Nothing was promoted out of it: every trap it carried is already in `docs/memory.md` (frozen
> subjects, mutation-red-first, a test that re-implements what it tests, reverting fixes one at a
> time, the architecture gate, the stamp gate, the blocking-commands flake). The one claim that was
> unique — that `.conducks/note-reviews.json` is committed — turned out to be FALSE when checked, and
> the reason is recorded in its own commit.
>
> Kept whole because it is a genuine record of how a long stretch of work actually went.
