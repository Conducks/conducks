# CONDUCKS — Conducks: Master Manifest 💎
...
## 🧬 1. The Conducks Pipeline (Orchestration)
...
sequenceDiagram
    ...
    participant PULSE as ConducksPipeline
...
## 💎 2. The 8 Unified Conducks Domains (58 Engines)

| Domain | Conducks Engines (The Core) | Terminal (Human) | Conducks Tool (Agentic Mode) |
|---|---|---|---|
| **1. Analysis** | `PulseOrchestrator`, `ConducksPipeline`, `NeuralReflector`, `SynapseRegistry`, `KahnTopoSorter`, `WasmProvider`, `RefractionEngine`, `SpectrumGenerator`, `SynapseEngine`, `ImportMap` | `conducks analyze` | `conducks_analyze` (graph_pulse) |
...
| **3. Governance**  | `ConducksSentinel`, `ConducksAdvisor`, `AuditEngine`, `PolicyRegistry`, `StatusEngine`, `AnomalyDetector`, `ConducksStalenessSensor` | `conducks verify`, `advise`, `status` | `conducks_governance` (graph_audit, advisor) |
...
| **7. System Mgmt** | `ConducksInstaller`, `MCPConfigurator`, `MirrorServer`, `BlueprintGenerator`, `ToolRegistry`, `HyperToonRegistry` | `conducks setup`, `mirror` | `conducks_system` (system_install, mirror) |
...
## ⚖️ 4. Conducks (Deterministic) vs Conducks (Heuristic)

| Feature | Conducks | CONDUCKS | Value |
|---|---|---|---|
