...
- Rule: a skill in `src/resources/skills/` states instructions ("confirm with grep before deleting"), not prohibitions; grounds a rule by naming its cost in the same sentence rather than citing a record number; and uses no path from this repository as though it were universal. Four skills ship: `conducks` (using the tool — what it is, the 14 MCP tools, the CLI, and the probe sequence per question), `conducks-docs` (the documentation standard), `conducks-visuals` (how the rendered pages are built), `conducks-feature-clean` (one door per feature, and cleaning behind it) — a new one needs a subject none of them covers. Retiring a skill adds its name to `RETIRED_SKILLS` in the installer so it is deleted from every scope on the next sync.
...
## CONDUCKS-48 — no numeric threshold after the registry
- Rule: a command or MCP tool may not hold a float comparison that decides anything. Thresholds are facts about a metric and live in `contracts` beside it, read by the domain that computes and the interface that renders, so the two cannot answer differently. Rendering choices — a colour picked from a value, a truncation for display — are not decisions and are granted by name in the gate.
- Reason: conducks is an octopus, and an arm that re-decides what the base already decided produces two answers to one question. `drift-engine.ts` counted `improvement_count` at `velocity < 0` while `drift.ts` listed at `< -0.01`, so sofie printed "Improving: 393" over an empty section; the partial-graph check lived only in `status.ts`, so `conducks_status` never warned an agent about a vault the CLI called broken. Both were found by grepping for what this rule forbids, not by reading the code. Enforced by `tests/architecture/no-logic-after-registry.test.ts` (ADR 0159).

## CONDUCKS-49 — a benchmark scenario is believed only after a mutation makes it fail
- Rule: every scenario in `tools/benchmark/bench-*.mjs` must be shown to go RED against a deliberate break in the analyzer it scores, and the break must fail the scenario written for it rather than some other one. A scenario that has only ever been green is unproven, whatever the tally says. State the mutation and which scenarios it fails in the ADR that adds the set.
- Reason: a benchmark is a claim about a tool and gets checked like one. Measured across the tool-verification campaign: **six scenarios were wrong before the tool was** — `bench-prune` scored 7/10 on its first run and all three failures were fixtures, and `bench-context` scenario 04 asserted an unrelated symbol is never a neighbour at radius 3 when it genuinely IS one three hops out. A green suite over wrong fixtures and a green suite over a correct tool are the same number. The mutation is what tells them apart, and it is also what catches the vacuous scenario — one that passes whether or not the mechanism exists (CONDUCKS-41 one level up). ADR 0189.
