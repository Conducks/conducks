...
Status: Accepted
- Enforced by: tests/unit/domain/docs/docs-grammar.test.ts (soft-default, architecture file-or-folder); tests/unit/domain/federation/installer-scope.test.ts ("uninstall clears every scope that has them and leaves foreign skills alone")
