...
- [ ] The standard itself is the one document nothing checks, and it drifted from the parser it describes. ADR 0034 widened `RE.task` to `[ xX>-]` and made a reasonless `[>]`/`[-]` fail lint; `src/resources/skills/conducks-docs.md` went on documenting two states, listed `space x X` as the whole marker set, and used `[-]` in §6 without ever defining it. Found by hand 2026-07-28 and fixed, third instance in two days. It cannot be governed today — `docs-lint` covers six file types (`todos` `decisions` `features` `conventions` `memory` `handover`) and the standard is none of them, living outside `docs/` entirely. So every drift between the standard and the parser is caught by a human or not at all. Decide what checks it: a test asserting the documented marker set equals `MARKER_TO_STATE`'s keys is the cheap 80%
