...
- [ ] The graph-walking paths still materialise: `conducks_status` pays 223 MB to report a node count that is `SELECT count(*)`, and impact/trace/flows walk in-memory adjacency where a recursive CTE would do. Converting those is what takes the remaining ~130 MB off a working session, and each one can move independently now that the deferral and the guard exist
