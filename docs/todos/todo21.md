...
- Builds: 0036
- [ ] `lazy` is a dead parameter — `registry-bootstrapper.ts` destructures it and never reads it, while `initializeRegistry(readOnly, root, lazy = readOnly)` plumbs it through two signatures. ATTEMPTED 2026-07-28 and REVERTED, with what it proved below. Verify it is still dead with `grep -n lazy src/lib/core/registry-bootstrapper.ts`
- [ ] MEASURED: deferring the load takes a read-only MCP server from 435 MB to **91 MB** — under this phase's target — and the deferral itself is easy. What is NOT easy is that almost nothing can use it yet: with the load skipped, four of six MCP tools broke and THREE broke SILENTLY. `conducks_status` reported `nodeCount: 0`, `conducks_flows` reported zero flows, `conducks_impact`/`conducks_trace` said SYMBOL_NOT_FOUND. No error anywhere — CONDUCKS-13 exactly
- [ ] A guard on the `graphEngine` accessor turns a missed call site into a loud throw, and it is necessary but NOT sufficient: the domain services (`governance`, `search`, `kinetic`, `metrics`) capture `graph.getGraph()` at construction, so they read the empty graph without ever touching the accessor. `conducks_status` stayed silently wrong with the guard in place. Any deferral must therefore land WITH the SQL conversion, not before it — a lazy load behind services that hold their own reference is a wrong-answer machine
- [ ] Deferring also needs a connection-lifecycle fix: the eager path closes the read-only persistence after loading, and a deferred load captures a persistence that is closed by the time it runs (`Database was already closed`). Decide who owns the connection across a deferred load before writing it again
- [ ] Only `conducks_docs` is graph-free today, and it already skips the anchor entirely — so deferral as a standalone change buys the memory back for docs-only sessions and nothing else. The win is the SQL conversion; the deferral is how it is collected
- [ ] Every read path walks JS objects instead of querying SQL, and that is what the 165 MB buys. The four MCP uses are a name lookup, a filtered scan, a bounded traversal and a grouping — `WHERE`, `WHERE`, a recursive CTE, and `GROUP BY`. `gravity`, `risk` and `canonicalRank` are already columns on `nodes`, written at analyze time, so nothing at query time needs the graph to derive them
- [ ] ADR 0036 states this and no phase claimed it: "pushing queries into DuckDB instead of walking a rebuilt in-memory graph is a separate change, and it is the one that makes split-second queries possible at all". Third instance of the defect in `todo22#P4` — an ADR consequence the board cannot see is unowned, found by hand again
- [ ] Decide what still justifies an in-memory graph after the read path leaves. Iterative whole-graph passes (PageRank/gravity at analyze time) are the honest candidate; a read-only server is not. If nothing does, say so and delete it rather than leaving it loaded "just in case"
- [ ] Eager loading costs 214 ms per command AND ~165 MB per process — the whole vault is deserialised before anything is answered. The memory is the half that bites, measured 2026-07-28: bare node is 45 MB, importing the registry makes it 68 MB, `initialize()` takes it to 233 MB for 2,381 nodes and 12,590 edges (~16 KB per node), and a working MCP server sits at 435 MB. The stdio transport spawns one server per client SESSION and the tool surface never parses anything (`registry.analyze.*` appears 0 times there, CONDUCKS-8), so every concurrent session pays a full graph load to answer read-only queries — three sessions is 1.3 GB. Grammars were measured and acquitted: 14 MB and 21 ms for all twelve. Fixed when a read-only MCP server answers `conducks_status` without materialising the graph, and its RSS after one query is under 100 MB
