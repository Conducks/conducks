...
- Builds: 0036
- [ ] `lazy` is a dead parameter. `registry-bootstrapper.ts:128` destructures it out of the options and never reads it again, while `initializeRegistry(readOnly, root, lazy = readOnly)` plumbs it through two signatures — so MCP already asks for a lazy load, gets an eager one, and nothing says so. Verify with `grep -n lazy src/lib/core/registry-bootstrapper.ts`: two hits, both the declaration. Fixed when the flag either changes behaviour or is gone
