...
- [ ] Where the ~165 MB actually goes, attributed step by step on this repo (rss / heap): baseline 58/6 → vault open 73/6 → SELECT nodes 105/16 → SELECT edges 113/26 → **addNode all 173/29** → addEdge all 188/44. The jump is `addNode`: +60 MB RSS against +3 MB heap, so it is V8 arena growth and adjacency-list Map overhead, not the rows. DuckDB's own buffers are ~37 MB of it. The node objects themselves are only 5.5 MB, of which 4.8 MB is the four `JSON.parse` calls per node (`metadata`, `kinetic`, `dna`, `signature`) — parsing those lazily is a real 8x on that slice and a small share of the total
- [ ] So the remaining win is a leaner in-memory representation, not a different query language: 2,402 nodes and 12,697 edges should not need 130 MB of arena. Decide what shape replaces object-per-node plus Maps — typed arrays with an id→index table is the obvious candidate — before writing anything. This is a bigger job than the three conversions that preceded it and should not be started on the assumption that it is the same kind of work
