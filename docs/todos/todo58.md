...
- [ ] DECIDE — and this one genuinely needs deciding, not a precedent lookup: teach the resolver to read `tsconfig` `rootDir`/`outDir`/`paths` and map a build-layout specifier back to source, or accept that seven findings on this subject stay wrong and say so in the docs so nobody re-derives it. Original framing kept below for the detail it carries:
