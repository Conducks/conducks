...
- [x] **Edge resolution for dynamic/interface/entry-point dispatch** — CLOSED 2026-08-01. It was marked PARTIAL, and every item under its own REMAINING list is now resolved: object-literal value capture DONE, DI dynamic-property chains DROPPED (their motivation was already decided against three lines below them), and the getters plus `initUI` DROPPED deliberately with a reason. A parent held open over children that are all closed is bookkeeping, not work. Original state (2026-07-19):
...
- [-] DROPPED 2026-08-01, Said's call, and the useful half ALREADY SHIPS. Two things were bundled here. COVERAGE CLICK-THROUGH IS DONE and was demonstrated rather than assumed: `conducks coverage <coverage-final.json>` binds a real jest run onto graph nodes — `83% PART clusterOf cluster-rule.ts:54-65 10/10 br`, with full/partial/dark counts. That is the static-⊕-coverage overlay working. What remains is the LIVE CROSS-SERVICE TRACE half: runtime proof that service A really called service B, rather than the static inference that it imports it. Dropped for three reasons. (1) It needs conducks to ship an instrumentation agent and a collector and to have a running instrumented target — that is a runtime-observability product, not a static analyser, and it is a different thing to build and maintain. (2) ADR 0013 already resolved ADR 0012's open reconcile and left this out by name. (3) The static surface it would confirm now EXISTS — route and request nodes are detected across all twelve grammars as of today (todo22#P15), so the question "which service calls which endpoint" is already answered statically; live tracing would only add "and it happened in production", which is an observability concern. If it is ever wanted it should be a new todo justified by a real target app, not this one
