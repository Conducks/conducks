...
- [ ] SCOPE, so this does not over-reach: stderr is CORRECT and must stay for the MCP server, where stdout is the JSON-RPC channel and stderr is the only legal log sink (`hypertoon.ts` uses it deliberately). The rule applies to the 32 CLI commands, not to `conducks mcp`
- [ ] Fixed when `node build/src/interfaces/cli/index.js status 2>&1 >/dev/null` produces zero lines on a healthy project, the same holds for the other read-only commands, and a flag or env var still surfaces the boot detail for debugging. A test asserting empty stderr is the gate; today it would fail with 20 lines
