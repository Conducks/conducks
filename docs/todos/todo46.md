...
- [ ] `conducks install-hooks [--force]` writes `.git/hooks/pre-commit` (or appends to an existing one behind a marked block, never silently replacing someone else's).
- [ ] The hook runs `docs-lint` and, when `docs/visuals/` exists, `visuals-lint` — each only when something relevant is staged, so an unrelated commit pays nothing.
- [ ] Idempotent: re-running changes nothing. A foreign hook is left alone with a printed instruction, not overwritten.
- [ ] Not a git checkout, or no `.git/hooks` — exit 0 quietly. A tarball install must not fail.
- [ ] `conducks setup` calls it, so adoption is one command.
