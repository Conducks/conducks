...
- [x] RE-MEASURED (ADR 0060), on `subject-b` rather than this repo because a subject with 326 commits exercises the git half the todo asks for. Peak 686 MB, and the shape is the finding: the write half takes it from 311 MB to 653 MB, the largest single jump being the wave-1 vault flush at +124 MB, while the whole read half adds 33 MB. Not comparable to the 881 MB figure, which was a different subject. Original: Re-measure the whole pulse. ADR 0042's prediction of roughly 500 MB is WITHDRAWN by 0043 — the peak is 881 MB today, down from 1,019 MB entirely through batching the per-row writes, and the read half is worth ~30 MB of what remains. Expect roughly 850 MB after Phases 1-3, and if it lands far from that, record the gap rather than adjusting the expectation quietly
