...
- [x] verified on two registered projects: it named exactly the five modules touched in this session, each against its own `MODULE.md`. Two defects found and fixed by that run — a false "1 gone" (a set difference counted every non-source file the pulse had hashed) and a leaked `fatal: not a git repository` from the non-git fallback
