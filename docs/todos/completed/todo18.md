...
- [x] the folder is out of the repo. It was MOVED to `<scratchpad>/legacy-removed-2026-07-26` rather than erased, because `rm -rf` is blocked in this session's sandbox — and it was untracked and gitignored, so git cannot restore it. Deleting that scratch copy is a one-liner for Said, carried in `docs/handover.md`, not work this repo is waiting on
