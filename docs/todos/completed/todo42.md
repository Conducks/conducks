...
- Acceptance: a call on a receiver that is a TYPED PARAMETER resolves to the type's declaration, an UNTYPED parameter is refused outright, and conducks' deep-chain dangling bucket falls below 59 — measured on conducks and subject-b together with precision and orphan count, per ADR 0077.
- Depends: none
...
- [x] Resolved (linker-intra 3b-quater): a bare `recv.member` whose receiver has no node reads the enclosing function's `paramTypes` — the map the three-segment chain (todo36) had read all along, never consulted for the plain two-segment shape. Measured on the frozen subjects: orchestrator dangling 1,938 -> 1,887, subject-c 3,361 -> 3,150, and subject-c's CALLS precision is 100.0% (0 wrong of 7,156 source-checked) with 324 more edges checkable
...
- [-] Measured on conducks and the three frozen subjects (baselines unchanged except intended doc gains; precision 99.93-99.99% held) — subject-b half dropped from this record: the machine does not hold that repo, a closed record cannot carry live work, and the machinery to run it later (tools/benchmark/doc-truth.mjs) is durable — re-raise as a fresh todo if subject-b ever lands here
