...
- [x] MEASURED on the frozen subjects: subject-a +2 nodes, orchestrator +7, subject-c +17, and **dangling unchanged on all three**. The nodes are locals that a correct rebind now references, so they survive the ATOM edge gate. `located` stays 100%. Baselines re-saved, warm and cold
