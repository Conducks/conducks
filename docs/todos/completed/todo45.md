...
- [x] Does the review key on the resolved file SET, or on each file individually? A set hash is one record and one clean signal; per-file records survive an anchor being added or removed without invalidating the whole page. Adding an anchor must not read as "the code changed" — decide which shape has that property and write it here. → answered FINER than either option: per resolved SPAN (`file`, `file:line`, `file:a-b`, `file::symbol` — ADR 0141/0142). Adding an anchor adds a stamp; removing one orphans its stamp VISIBLY; nothing invalidates the rest.
- [x] Is a drifted visual a WARN or an ERROR? ADR 0138 fails the run only on broken anchors, on the argument that a heuristic must not block a commit. Drift is not a heuristic, but it is also not proof the page is wrong. Record the answer and the reason. → WARN (ADR 0141): the cited code changing is not proof the claim is false — only a reader can judge tier three, so the flag's job is a short precise re-read list, never a blocked commit.
...
- [x] A page that has NEVER been reviewed is not reported as drifted. Flagging every page on a first run makes the command noise, and noise is how a gate stops being read (the same reasoning `driftedReviews` already carries). → an unstamped anchor is never flagged; stamping IS the review (ADR 0141), pinned by test.
- [x] Proven: an unreviewed page produces no drift line; a reviewed-then-changed page names exactly the changed files. → both pinned in the visuals-lint suite; live-proven on conducks' own 58-stamp baseline.
