...
export class CohesionCommand implements ConducksCommand {
  ...
  public async execute(args: string[], registry: Registry): Promise<void> {
    ...
    try {
      ...
      console.log(`\x1b[2mA:\x1b[0m ${displayId(String(a ?? ''), cohesionRoot, nameLookupFrom(graph))}`);
      console.log(`\x1b[2mB:\x1b[0m ${displayId(String(b ?? ''), cohesionRoot, nameLookupFrom(graph))}`);
