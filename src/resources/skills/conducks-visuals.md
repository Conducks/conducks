<!-- description: How the rendered architecture pages are built and what the build refuses — the tree of pages, the band-by-band growth, data/layout separation, anchors and the gate, routing, occlusion, porting the generator into a new repo, and the testing page a human works through by hand. Use when creating, extending, reviewing or porting visuals, when building or updating a manual test page, or when visuals-lint fails. Reach for it whenever something will be LOOKED at rather than read — even when the user never says "visual": "draw the architecture", "show me how it fits together", "make a diagram", "put it on the canvas", "a page someone can click through". `conducks-docs` §6.13 owns WHEN a visual may exist; this owns how it is built and what the build refuses. -->
