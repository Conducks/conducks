...
export class DuckDbPersistence implements SynapsePersistence {
  ...
  constructor(baseDir?: string, readOnly: boolean = false) {
    ...
    if (projectRoot === ":memory:") {
      ...
      console.error(`[Conducks Persistence] Initialized at: ${projectRoot}`);
    }
  }
      ...
      try {
        const db = new duckdb.Database(this.dbPath, (err: any) => {
          ...
          db.serialize(() => {
            ...
            const nodeSchema = `CREATE TABLE IF NOT EXISTS nodes (id TEXT, pulseId VARCHAR, kind TEXT, name TEXT, file VARCHAR, gravity DOUBLE, kineticEnergy DOUBLE, frameworks VARCHAR, isTest BOOLEAN DEFAULT FALSE, complexity INTEGER DEFAULT 1, debtMarkers VARCHAR, resonance INTEGER DEFAULT 0, entropy DOUBLE DEFAULT 0, primaryAuthor VARCHAR, authorCount INTEGER DEFAULT 0, lastModified INTEGER DEFAULT 0, tenureDays INTEGER DEFAULT 0, coveredBy VARCHAR, anomaly VARCHAR, isEntryPoint BOOLEAN DEFAULT FALSE, ecosystem VARCHAR, version VARCHAR, metadata VARCHAR, PRIMARY KEY (id, pulseId));`;
