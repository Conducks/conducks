...
export class ConducksGraph {
  ...
  private bindPulseCircuits(): void {
    ...
    for (const node of allNodes) {
      ...
      const assignments = outgoing.filter((e: any) => e.properties.reason === 'assignment');
      const calls = outgoing.filter((e: any) => e.type === 'CALLS' && !e.properties.isResonance);
  ...
  private bindRouteCircuits(): void {
    const allNodes = Array.from(this.graph.getAllNodes());
    const routes = allNodes.filter((n: any) => n.properties.isRoute);
    const requests = allNodes.filter((n: any) => n.properties.isRequest);
    ...
    for (const req of requests) {
      ...
      for (const route of routes) {
        ...
        if ((req as any).properties.method === (route as any).properties.method && this.isUrlMatch((req as any).properties.url, (route as any).properties.path)) {
          const edge: ConducksEdge = {
            id: `RESONANCE::${(req as any).id}->${(route as any).id}`,
            sourceId: (req as any).id,
            targetId: (route as any).id,
            type: 'CALLS' as any, // In v6, resonance is a high-level CALL
            confidence: 0.9,
            properties: { isResonance: true, url: (req as any).properties.url }
          };
          this.graph.addEdge(edge);
        }
      }
    ...
    for (const node of allNodes) {
      ...
      for (const edge of outgoing) {
        if (edge.type === 'CALLS' || edge.type === 'ACCESSES' || edge.type === 'CONSTRUCTS' || edge.type === 'MEMBER_OF') {
          ...
          const fileId = (node as any).properties.filePath;
          const globalId = `${fileId}::global`;
          const localImports = outgoing.filter((e: any) => e.type === 'IMPORTS');
          const fileImports = this.graph.getNeighbors(globalId, 'downstream').filter((e: any) => e.type === 'IMPORTS');
