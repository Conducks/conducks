...
export class ManifestEngine implements ConducksComponent {
  ...
  /**
   * The conducks-docs grammar file set (see the conducks-docs skill §4 and docs-grammar.ts —
   * the SAME grammar `docs-lint` validates). Every scaffolded file MUST pass docs-lint, or the
   * bootstrap generates docs the formatter rejects. Flat docs/ — never nested per-project dirs.
   * architecture.md is DERIVED (context-gen owns it); decisions/ is authored on demand — neither
   * is scaffolded with authored content.
   */
  private grammarFiles(projectName: string): Array<{ name: string; content: string }> {
    return [
      ...
      { name: 'architecture.md', content: `# Architecture — ${projectName} · generated by conducks\n<!-- DERIVED. Do not edit — run \`conducks context-gen --out docs/architecture.md\`. Intent → features.md. -->\n` },
    ];
  }
