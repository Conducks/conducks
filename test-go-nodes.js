...
async function run() {
  try {
    ...
    const goWasmPath = path.resolve(__dirname, 'build/src/resources/grammars/tree-sitter-go.wasm');
    
    await Parser.init({
      locateFile: (file) => file === 'tree-sitter.wasm' ? wasmPath : file
    });
    
    const lang = await Parser.Language.load(fs.readFileSync(goWasmPath));
